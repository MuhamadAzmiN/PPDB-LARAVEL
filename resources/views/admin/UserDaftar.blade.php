        ...
        <div class="sm:flex sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-x-3">
                    <h2 class="text-lg font-medium text-gray-800 dark:text-white"> User Daftar</h2>
                            ...
                            @foreach($daftar as $item)
                                @if($item)
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-12 py-4 text-sm font-medium whitespace-nowrap">{{ $item->user->name }}</td>
                                        <td class="px-4 py-4 text-sm whitespace-nowrap">{{ $item->user->email }}</td>
                                        ...
                                        <td class="px-4 py-4 text-sm whitespace-nowrap">
                                            ...
                                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                <form action="{{ route('user.delete', ['id' => $item->id]) }}" method="POST">
                                                    @method('delete')
                                                    @csrf
                                                    <button type="submit" class="text-yellow-300">Hapus</button>
                                                </form>
                                            <a class="text-yellow-300" href="{{ route('item.detail', ['id' => $item->id]) }}">Detail</a>
                                            <form action="{{ route('user.daftar', ['id' => $item->id]) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="text-yellow-300">Lulus</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endif
